...
struct FatPipeConfig: Decodable {
...
class ConfigManager: ObservableObject {
    ...
    func checkForUpdates() {
        loadJson(fromURLString: "https://simplejsoncms.com/api/lj7apqugvv") { result in
            switch result {
            case let .success(data):
                //                    print("Got data")
                let config: FatPipeConfig = try! JSONDecoder().decode(FatPipeConfig.self, from: data)
...
//                    print("User is using an older version of the app")
                    DispatchQueue.main.async { [unowned self] in
                        ...
                        alert.informativeText = "A new version of Speedy is available."
